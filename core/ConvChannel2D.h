...
public:
	...
	ConvChannel2D(const ConvFilter2D& _filter, 
				const F& _rand_scale, 
				const F& _rand_min, 
				const ConvImage2D& _input_image, 
				const int& _in_offset, 
				const ConvImage2D& _output_image, 
				const int& _out_offset) {
		initialize(_filter, _rand_scale, _rand_min, _input_image, _in_offset, _output_image, _out_offset);
	}

	void initialize(const ConvFilter2D& _filter, 
					const F& _rand_scale, 
					const F& _rand_min, 
					const ConvImage2D& _input_image, 
					const int& _in_offset, 
