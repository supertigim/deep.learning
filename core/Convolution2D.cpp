...
namespace Convolution2D {
	...
	// optimized resolution of output
	int getDesiredOutputRes(const int& width_in, 
	...
	void forward(const ConvFilter2D& filter, 
				const ConvImage2D& input_image, 
				ConvImage2D& output_image) {
	...
	{
		VectorND<F> filter_weights_temp(filter.i_res_ * filter.j_res_ + 1);
		filter_weights_temp.copyPartial(filter.weights_, 0, 0, filter.weights_.num_dimension_);
		filter.weights_.assignAllValues((F)0);
